...
@Component({
  selector: 'app-beat-summary',
  templateUrl: './beat-summary.component.html',
  styleUrls: ['./beat-summary.component.scss']
})
export class BeatSummaryComponent implements OnInit {
  ...
  eventTrg(event) {
    ...
    if(ngValue === '2') {
      ...
      this.router.navigate(['/main/reports/spduty-summary']);
    }
  }
  ddFoUser() {
  ...
  ddgetBeat() {
    this.beatService.getBeatType().subscribe((resp: any) => {
      ...
      this.typeBeat.unshift({id: 'all', displayName: 'All', key: 'All'})
  ...
  onSubmit() {
    ...
    if (this.reportForm.valid) {
      ...
      this.formService.getBeatReportSummary(this.bType).subscribe((resp: APIResponse) => {
        this.ksLoader = true;
